//! The `CpuBackend` implementation.

use crate::kernels::{
  attention::attention_step, embedding::embedding_row, matmul, norm::rmsnorm_row,
  rope::rope_inplace, sampling::sample, softmax::softmax_rows_inplace, swiglu::silu_gate,
};
...
// ---- Helpers to reinterpret TensorRef payloads ----
...
fn launch_attention(
  ...
  let k = as_f32_slice(&inputs[1])?;
  let v = as_f32_slice(&inputs[2])?;
...
fn launch_reshape(inputs: &[TensorRef], outputs: &mut [TensorMut]) -> Result<()> {
  // Contiguous reshape is a bytewise copy.
  if inputs.is_empty() || outputs.is_empty() {
...
fn launch_cast(attrs: &OpAttrs, inputs: &[TensorRef], outputs: &mut [TensorMut]) -> Result<()> {
  ...
  let (y_out, _) = outputs.split_first_mut().unwrap();
  let src_dtype = inputs[0].dtype().ok_or_else(|| FellmError::other("cast src dtype"))?;
  let dst_dtype = attrs
      .cast_dtype()
